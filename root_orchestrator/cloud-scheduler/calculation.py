...
def get_manifest(image_ref, registry_url):
    ...
    print("Registry url is ", registry_url)
...
def memory_estimation(available_memory, image, memory, storage=None):
    ...
    if not image or image == "noImageDetected":
        # Avoid trying to estimate image size if no image is defined
        print("No image defined in job description")
        return available_memory >= memory

    # Get and convert image size to GB
...
def does_cluster_respects_requirements(cluster, job):
    ...
    if virtualization == "unikernel":
        ...
        # Check if explicit memory (nb. at moment storage, can be changed later)
        ...
        if (
            cluster_specs["available_cpu"] >= vcpu
            and memory_estimation(cluster_specs["available_memory"], image, memory, storage)
            and virtualization in cluster_specs["virtualization"]
            and cluster_specs["available_gpu"] >= vgpu
        ):
            return True
